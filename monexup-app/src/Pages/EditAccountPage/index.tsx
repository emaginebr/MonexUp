...
/**
 * EditAccountPage — redesigned `/admin/edit-account` route.
 *
 * Visual contract: matches the redesigned ProfileEditPage / ProfileListPage
 ...
 *   - delegates load + save to nauth-react's `UserEditForm` keyed by
 *     `authContext.sessionInfo?.userId`
 *   - success / error are routed through the existing `MessageToast`
 *   - `Voltar` returns to `/admin/dashboard`
 *
 * UX wiring: nauth's `UserEditForm` renders its own `<form>` and submit
 * button. We hide that inline button via `[&_form_button[type=submit]]:hidden`
 * (the form orange skin still applies via `nauth-overrides.scss` — but we
 * don't show it). The header `Salvar` button locates the rendered `<form>`
 * via the wrapper ref and triggers `requestSubmit()` so native HTML5
 * validation + RHF resolver still fire.
 */
export default function EditAccountPage() {
  ...
  const userId = authContext.sessionInfo?.userId;
  ...
  const handleHeaderSave = () => {
    ...
    // resolver validation still runs inside UserEditForm.
    formEl.requestSubmit();
  };

  const headlineText =
    authContext.sessionInfo?.name?.trim() || t("edit_account");
    ...
    <main className="mnx-surface-light bg-mnx-neutral-50 min-h-screen">
      ...
      <div className="max-w-container mx-auto px-shell pt-6 pb-12">
        ...
        <section
          ...
          aria-labelledby="edit-account-page-title"
        >
