...
export interface OrderSearchRowLabels {
  /** dl labels (mobile) */
  buyerLabel: string;
  productLabel: string;
  totalLabel: string;
  ...
  viewDetails: string;
  /** Translated status text for this order. */
  statusText: string;
}

export interface OrderSearchRowProps {
  order: OrderInfo;
  labels: OrderSearchRowLabels;
  /** Where the row's view-details button links to. */
  detailsHref: string;
  /** Number formatter for the total amount. */
  formatTotal: (value: number) => string;
  /** Date formatter for createdAt / updatedAt. */
  formatDate: (iso: string) => string;
}

/**
 * OrderSearchRow — single `/admin/orders` entry rendered in two layouts:
 *
 *   • md+   : a 12-column grid row inside the card table.
 *             Order#code+date 3 / Buyer name+email 3 / Product 2 /
 *             Total (right) 2 / Status pill (right) 1 / Actions 1.
 *   • <md   : a stacked card. Order code + status pill on top,
 *             buyer + product subtitle, total + last-change footer,
 *             action cluster on the right of the title row.
 *
 * Pure presentational. Parent owns the data flow and formatting helpers
 * so the legacy behavior (totals from items[], showProducts join,
 * Moment-style updatedAt) is preserved without touching anything else.
 */

function statusPillClasses(status: OrderStatusEnum): string {
  // Editorial-brutalist tints to match ProfileListPage / UserSearchRow.
  switch (status) {
...
function StatusIcon({ status }: { status: OrderStatusEnum }) {
  switch (status) {
    ...
    case OrderStatusEnum.Suspended:
      return <PauseCircle size={12} aria-hidden="true" />;
    case OrderStatusEnum.Expired:
...
export default function OrderSearchRow({
  order,
  labels,
  detailsHref,
  formatTotal,
  ...
  // Build product join string (preserves legacy showProducts).
  const productLine =
  ...
  // Build total (preserves legacy showTotal).
  const totalAmount =
