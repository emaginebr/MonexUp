...
| `vendor-storefront-product.html`                     | Static HTML/CSS mockup of `/{store-slug}/store/{vendor-slug}` (product page) — 2 art directions (A Editorial Premium · B Vibrant Social), 2 states each (fixed price + PIX + Comprar / open donation + Card + Doar). Vendor-branded surface; MonexUp reduced to a condensed footer. | `ui-ux-pro-max`    |
| `vendor-storefront-product.md`                       | Companion notes for the vendor storefront product page: direction rationale, palette, type, layout, data binding to existing `StorefrontProductInfo` | `ui-ux-pro-max`    |
| `vendor-storefront-list.html`                        | Static HTML/CSS mockup of `/{networkSlug}/store/{sellerSlug}` (product listing) — same 2 art directions inherited from the detail page, 2 states each (full grid 3×3 with pagination · empty list). Cards link through to the detail page route. | `ui-ux-pro-max`    |
| `vendor-storefront-list.md`                          | Companion notes for the vendor storefront product list: grid breakpoints, card aspect ratio per direction, pagination behavior, empty-state rules, a11y inheritance from the detail page | `ui-ux-pro-max`    |
